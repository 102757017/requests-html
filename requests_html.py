...
class BaseSession(requests.Session):
    ...
    def __init__(self, mock_browser : bool = True, verify : bool = True,
                 browser_args : list = ['--no-sandbox'],headless=True,autoClose=True,executablePath=r"C:\Program Files\Google\Chrome\Application\chrome.exe"):
        super().__init__()
    ...
    async def browser(self):
        if not hasattr(self, "_browser"):
            self._browser = await pyppeteer.launch(ignoreHTTPSErrors=not(self.verify), headless=self.__headless,autoClose=self.__autoClose, executablePath=self.__executablePath,args=self.__browser_args)
